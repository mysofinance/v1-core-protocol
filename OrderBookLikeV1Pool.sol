...
contract Contract {
    ...
    function claim(uint256 _slotIdx, uint256[] calldata _loanIdxs) external {
        ...
        (uint256 repayments, uint256 collateral, uint256 checkInAll) = repayCollAndCheckMask(checkSlot, _loanIdxs, lpInfo.weight);
    ...
    function claimOnGroup(uint256 _slotIdx, uint256[] calldata _slots, uint256[] calldata _loanIdxs) external {
        uint256 slotArrayLen = _slots.length;
        require(slotArrayLen > _slotIdx && slotArrayLen < 256, "slotArrayLen out of bounds");
        ...
        require(loanArrayLen > 0 && loanArrayLen < loanIdx, "loanArrayLen out of bounds");
        GroupedClaimsInfo memory claimsInfo = slotsToClaimsInfo[keccak256(abi.encodePacked(_slots,_loanIdxs))];
        lpToMaxLoanIdxClaimed[keccak256(abi.encodePacked(_slotIdx,msg.sender))] = _loanIdxs[loanArrayLen-1];
        uint256 repayments = claimsInfo.repayments * lpInfo.weight / claimsInfo.groupWeight;
        uint256 collateral = claimsInfo.collateral * lpInfo.weight / claimsInfo.groupWeight;
        //ERC20 transfer repayments and collateral
    }

    function groupClaims(uint256[] calldata _slots, uint256[] calldata _loanIdxs) external {
        (uint128 groupWeight, uint256 slots) = weightsAndSlots(_slots);
        (uint256 repayments, uint256 collateral, uint256 checkMask) = repayCollAndCheckMask(slots, _loanIdxs, groupWeight);
        ...
        slotsToClaimsInfo[keccak256(abi.encodePacked(_slots,_loanIdxs))] = claimsInfo;
    }

    function weightsAndSlots(uint256[] calldata _slots) internal view returns (uint128, uint256) {
        uint256 slotArrayLen = _slots.length;
    ...
    function repayCollAndCheckMask(uint256 _slots, uint256[] calldata _loanIdxs, uint256 _weight) internal view returns (uint256, uint256, uint256) {
        uint256 loanArrayLen = _loanIdxs.length;
        require(loanArrayLen > 0 && loanArrayLen < loanIdx, "loanArrayLen out of bounds");
        LoanInfo memory loanInfo;
        require(block.number > loanInfo.expiry, "loans must have expired");
        ...
        for (uint128 i = 0; i < loanArrayLen; ++i) {
            ...
            if (loanInfo.repaid) {
                repayments += loanInfo.repayment * _weight / loanInfo.totalWeights;
            } else {
                collateral += loanInfo.collateral * _weight / loanInfo.totalWeights;
            }
            prev = _loanIdxs[i];
        }
