...
describe("RPL-USDC Pool Testing", function () {
  ...
  it("Test core functions", async function () {
    const [USDC, RPL, pool, deployer, lp1, lp2, lp3, borrower1, borrower2] =
      await setup();
    ...
    expect(lp1UsdcPreAddCheck.sub(lp1UsdcPostAddCheck)).to.be.equal(
      poolUsdcPostCheck.sub(poolUsdcPreCheck)
    );
    ...
    expect(lp1UsdcPostRemoveCheck.sub(lp1UsdcPreRemoveCheck)).to.be.equal(
      poolInfo._totalLiquidity.sub("10000000")
    ); // sub minLiquidity
  });
});
