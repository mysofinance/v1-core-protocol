...
describe("RPL-USDC Pool Testing", function () {
  ...
  async function setupAccsAndTokens() {
    ...
    await RPL.connect(rplHolder).transfer(
      borrower1.address,
      ONE_RPL.mul(1000000)
    );
    await RPL.connect(rplHolder).transfer(
  ...
  async function setup() {
    ...
    const [USDC, RPL, deployer, lp1, lp2, lp3, borrower1, borrower2] =
      await setupAccsAndTokens();
  ...
  it("Test core functions", async function () {
    const [USDC, RPL, pool, deployer, lp1, lp2, lp3, borrower1, newCreator] =
      await setup();
    ...
    await expect(
      pool.connect(lp1).addLiquidity(lp1.address, addAmount, timestamp + 60, 0)
    ).to.be.revertedWithCustomError(pool, "UnapprovedSender");
    await expect(
      pool.connect(lp1).toggleLpWhitelist(lp1.address)
    ).to.be.revertedWithCustomError(pool, "UnapprovedSender");
    await expect(
      pool.connect(lp2).toggleLpWhitelist(lp1.address)
    ).to.be.revertedWithCustomError(pool, "UnapprovedSender");
    await pool.connect(deployer).toggleLpWhitelist(lp1.address);
    await pool
      .connect(lp1)
      .addLiquidity(lp1.address, addAmount, timestamp + 60, 0);

    // check proposing and claiming pool creator role
    await expect(
      pool.connect(lp1).proposeNewCreator(newCreator.address)
    ).to.be.revertedWithCustomError(pool, "UnapprovedSender");
    await expect(
      pool.connect(lp1).claimCreator()
    ).to.be.revertedWithCustomError(pool, "UnapprovedSender");
    await pool.connect(deployer).proposeNewCreator(newCreator.address);
    await expect(
      pool.connect(lp2).claimCreator()
    ).to.be.revertedWithCustomError(pool, "UnapprovedSender");
    await pool.connect(newCreator).claimCreator();

    // check lpwhitelisting only possible for new creator
    await expect(
      pool.connect(deployer).toggleLpWhitelist(lp2.address)
    ).to.be.revertedWithCustomError(pool, "UnapprovedSender");
    await expect(
      pool.connect(lp2).addLiquidity(lp2.address, addAmount, timestamp + 60, 0)
    ).to.be.revertedWithCustomError(pool, "UnapprovedSender");
    await pool.connect(newCreator).toggleLpWhitelist(lp2.address);
    await expect(
      pool.connect(lp1).addLiquidity(lp2.address, addAmount, timestamp + 60, 0)
    ).to.be.revertedWithCustomError(pool, "UnapprovedSender");
    await expect(
      pool.connect(lp2).addLiquidity(lp1.address, addAmount, timestamp + 60, 0)
    ).to.be.revertedWithCustomError(pool, "UnapprovedSender");
    await pool
      .connect(lp2)
      .addLiquidity(lp2.address, addAmount, timestamp + 60, 0);
    await pool.connect(newCreator).toggleLpWhitelist(lp2.address);
    await expect(
      pool.connect(lp2).addLiquidity(lp2.address, addAmount, timestamp + 60, 0)
    ).to.be.revertedWithCustomError(pool, "UnapprovedSender");
    ...
    expect(
      lp1UsdcPreAddCheck
        .sub(lp1UsdcPostAddCheck)
        .add(lp2UsdcPreAddCheck.sub(lp2UsdcPostAddCheck))
    ).to.be.equal(poolUsdcPostCheck.sub(poolUsdcPreCheck));
    ...
    console.log(
      `PLEDGE AMOUNT (RPL); LOAN AMOUNT (USDC); POOL UTIL.; APR; LTV`
    );
    for (let i = 0; i < sendAmountMultiple.length; i++) {
      ...
      const tmpUtil = tmpLoanTerms.loanAmount
        .mul(10000)
        .div(tmpPoolInfo._totalLiquidity);
      const tmpApr = tmpLoanTerms.repaymentAmount
        .sub(tmpLoanTerms.loanAmount)
        .mul(60 * 60 * 24 * 365)
        .mul(10000)
        .div(tmpPoolInfo._loanTenor)
        .div(tmpLoanTerms.loanAmount);
      const tmpLtv = tmpLoanTerms.loanAmount
        .mul(COLL_TOKEN_DECIMALS)
        .mul(10000)
        .mul(100)
        .div(tmpSendAmount)
        .div(RPL_PRICE_IN_CENT)
        .div(LOAN_TOKEN_DECIMALS);
      console.log(
        `${Number(tmpSendAmount.div(ONE_RPL.div(100).toString())) / 100}; ${
          tmpLoanTerms.loanAmount.div(ONE_USDC.div(100).toString()) / 100
        }; ${Number(tmpUtil) / 100}%; ${Number(tmpApr) / 100}%; ${
          Number(tmpLtv) / 100
        }%`
      );
    }
    ...
    expect(poolRateParamsPost._liquidityBnd1).to.be.equal(
      newTerms.liquidityBnd1
    );
    expect(poolRateParamsPost._liquidityBnd2).to.be.equal(
      newTerms.liquidityBnd2
    );
    ...
    expect(loanTermsPost._creatorFee).to.be.equal(
      newTerms.creatorFee.mul(sendAmount).div(BASE)
    );
    ...
    const borrower1UsdcPostBorrowCheck = await USDC.balanceOf(
      borrower1.address
    );
    ...
    expect(
      borrower1UsdcPostBorrowCheck.sub(borrower1UsdcPreBorrowCheck)
    ).to.be.equal(poolUsdcPreBorrowCheck.sub(poolUsdcPostBorrowCheck));
    expect(
      borrower1UsdcPostBorrowCheck.sub(borrower1UsdcPreBorrowCheck)
    ).to.be.equal(loanTermsPost.loanAmount);
    expect(
      borrower1RplPreBorrowCheck
        .sub(borrower1RplPostBorrowCheck)
        .sub(loanTermsPost._creatorFee)
    ).to.be.equal(poolRplPostBorrowCheck.sub(poolRplPreBorrowCheck));
    expect(
      borrower1RplPreBorrowCheck
        .sub(borrower1RplPostBorrowCheck)
        .sub(loanTermsPost._creatorFee)
    ).to.be.equal(loanTermsPost.pledgeAmount);
    expect(creatorRplPostBorrow.sub(creatorRplPreBorrow)).to.be.equal(
      loanTermsPost._creatorFee
    );

    // check repay
    await USDC.connect(lp1).transfer(
      borrower1.address,
      loanTermsPost.repaymentAmount
    );
    ...
    await pool
      .connect(borrower1)
      .repay(1, borrower1.address, loanTermsPost.repaymentAmount);
    ...
    expect(
      borrower1UsdcPreRepayCheck.sub(borrower1UsdcPostRepayCheck)
    ).to.be.equal(poolUsdcPostRepayCheck.sub(poolUsdcPreRepayCheck));
    expect(
      borrower1UsdcPreRepayCheck.sub(borrower1UsdcPostRepayCheck)
    ).to.be.equal(loanTermsPost.repaymentAmount);
    expect(
      borrower1RplPostRepayCheck.sub(borrower1RplPreRepayCheck)
    ).to.be.equal(poolRplPreRepayCheck.sub(poolRplPostRepayCheck));
    expect(
      borrower1RplPostRepayCheck.sub(borrower1RplPreRepayCheck)
    ).to.be.equal(loanTermsPost.pledgeAmount);
    ...
    expect(lp1UsdcPostClaimCheck.sub(lp1UsdcPreClaimCheck)).to.be.equal(
      poolUsdcPreClaimCheck.sub(poolUsdcPostClaimCheck)
    );
    expect(lp1UsdcPostClaimCheck.sub(lp1UsdcPreClaimCheck)).to.be.equal(
      loanTermsPost.repaymentAmount.div(2)
    );
    ...
    expect(lp1UsdcPostRemoveCheck.sub(lp1UsdcPreRemoveCheck)).to.be.equal(
      poolUsdcPreRemoveCheck.sub(poolUsdcPostRemoveCheck)
    );
    expect(lp1UsdcPostRemoveCheck.sub(lp1UsdcPreRemoveCheck)).to.be.equal(
      poolInfo._totalLiquidity.sub("10000000").div(2)
    ); // sub minLiquidity
    ...
    expect(lp2UsdcPostClaimCheck.sub(lp2UsdcPreClaimCheck)).to.be.equal(
      poolUsdcPreClaimCheck_.sub(poolUsdcPostClaimCheck_)
    );
    expect(lp2UsdcPostClaimCheck.sub(lp2UsdcPreClaimCheck)).to.be.equal(
      loanTermsPost.repaymentAmount.div(2)
    );
    ...
    await pool
      .connect(lp2)
      .removeLiquidity(lp2.address, lpInfo_.sharesOverTime[0]);
    ...
    expect(lp2UsdcPostRemoveCheck.sub(lp2UsdcPreRemoveCheck)).to.be.equal(
      poolUsdcPreRemoveCheck_.sub(poolUsdcPostRemoveCheck_)
    );
    expect(lp2UsdcPostRemoveCheck.sub(lp2UsdcPreRemoveCheck)).to.be.equal(
      poolInfo_._totalLiquidity.sub("10000000")
    ); // sub minLiquidity
  });
});
