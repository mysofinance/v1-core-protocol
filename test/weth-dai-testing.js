...
describe("WETH-DAI Pool Testing", function () {
  ...
  it("Should fail on loan terms without LPs", async function () {
    await expect(poolWethDai.loanTerms(ONE_ETH)).to.be.revertedWithCustomError(
      poolWethDai,
  ...
  it("Should correctly calculate loan terms", async function () {
    ...
    await poolWethDai
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_DAI.mul(2000000), timestamp + 60, 0);
    ...
    expect(loanTerms.repaymentAmount).to.be.equal(
      "224242007868923219707998854"
    );
    tmpRate = loanTerms.repaymentAmount.mul(10000).div(loanTerms.loanAmount);
    tmpRate = tmpRate.toNumber();
    expect(tmpRate).to.be.greaterThan(rate); // compare APR with APR of smaller loan; rate should be increasing outside of constant APR range
  ...
  it("Should allow LPs to add liquidity", async function () {
    ...
    await poolWethDai
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_DAI.mul(1111), timestamp + 60, 0);
    await poolWethDai
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_DAI.mul(10111), timestamp + 60, 0);
    await poolWethDai
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_DAI.mul(130111), timestamp + 60, 0);
    poolInfo = await poolWethDai.getPoolInfo();
  ...
  it("Should allow borrowing with ETH", async function () {
    ...
    await poolWethDai
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_DAI.mul(1000), timestamp + 60, 0);
    await poolWethDai
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_DAI.mul(10000), timestamp + 60, 0);
    await poolWethDai
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_DAI.mul(100000), timestamp + 60, 0);
  ...
  it("Should not allow new LPs to claim on unentitled previous loans", async function () {
    ...
    await poolWethDai
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_DAI.mul(30000), timestamp + 60, 0);
    await poolWethDai
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_DAI.mul(20000), timestamp + 60, 0);
    await poolWethDai
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_DAI.mul(10000), timestamp + 60, 0);
    ...
    await poolWethDai
      .connect(lp4)
      .addLiquidity(lp4.address, ONE_DAI.mul(1000), timestamp + 60, 0);
    await expect(
      poolWethDai
        .connect(lp4)
  ...
  it("Should be possible to borrow when there's sufficient liquidity, and allow new LPs to add liquidity to make borrowing possible again", async function () {
    ...
    await poolWethDai
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_DAI.mul(1000), timestamp + 60, 0);
  ...
  it("Should allow LPs to claim individually", async function () {
    ...
    await poolWethDai
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_DAI.mul(100000), timestamp + 60, 0);
    await poolWethDai
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_DAI.mul(100000), timestamp + 60, 0);
    await poolWethDai
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_DAI.mul(100000), timestamp + 60, 0);
  ...
  it("Should handle aggregate claims correctly (1/2)", async function () {
    ...
    await poolWethDai
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_DAI.mul(500000), timestamp + 60, 0);
    await poolWethDai
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_DAI.mul(500000), timestamp + 60, 0);
    await poolWethDai
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_DAI.mul(300000), timestamp + 60, 0);
    await poolWethDai
      .connect(lp4)
      .addLiquidity(lp4.address, ONE_DAI.mul(200000), timestamp + 60, 0);
  ...
  it("Should handle aggregate claims correctly (2/2)", async function () {
    ...
    await poolWethDai
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_DAI.mul(300000), timestamp + 60, 0);
    await poolWethDai
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_DAI.mul(200000), timestamp + 60, 0);
  ...
  it("Should allow adding liquidity again after removing and claiming", async function () {
    ...
    await poolWethDai
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_DAI.mul(500000), timestamp + 1000, 0);
    ...
    await expect(poolInfo._totalLpShares).to.be.equal(
      ONE_DAI.mul(500000).mul(1000).div(minLiquidity)
    );
  });
  ...
  it("Should never fall below minLiquidity", async function () {
    ...
    await poolWethDai
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_DAI.mul(1001), timestamp + 60, 0);
  ...
  it("Shouldn't overflow even after 4x rounds of consecutive LPing with DAI ≈80mn and borrowing against 120,000,000 ETH", async function () {
    ...
    await poolWethDai
      .connect(lp1)
      .addLiquidity(lp1.address, bal, timestamp + 1000000000, 0);
    loanTerms = await poolWethDai.loanTerms(pledgeAmount);
    console.log(loanTerms);
    await poolWethDai
      .connect(borrower)
    ...
    await poolWethDai
      .connect(lp2)
      .addLiquidity(lp2.address, bal, timestamp + 1000000000, 0);
    loanTerms = await poolWethDai.loanTerms(pledgeAmount);
    console.log(loanTerms);
    await poolWethDai
      .connect(borrower)
    ...
    await poolWethDai
      .connect(lp3)
      .addLiquidity(lp3.address, bal, timestamp + 1000000000, 0);
    loanTerms = await poolWethDai.loanTerms(pledgeAmount);
    console.log(loanTerms);
    await poolWethDai
      .connect(borrower)
    ...
    await poolWethDai
      .connect(lp4)
      .addLiquidity(lp4.address, bal, timestamp + 1000000000, 0);
    loanTerms = await poolWethDai.loanTerms(pledgeAmount);
    console.log(loanTerms);
    await poolWethDai
      .connect(borrower)
    ...
    for (let i = 0; i < 100; i++) {
      ...
      if (i == 0) {
        ...
        newLpShares = lpDaiBal
          .mul(poolInfo._totalLpShares)
          .div(poolInfo._totalLiquidity);
      ...
      try {
        await poolWethDai
          .connect(lp1)
          .addLiquidity(lp1.address, lpDaiBal, timestamp + 1000000000, 0);
        counter += 1;
      } catch (error) {
        await expect(
          poolWethDai
      ...
      pledgeAmount = ethers.BigNumber.from("0x204FCE5E3E25026110000000").sub(
        ONE_ETH
      );
  ...
  it("(2/2) Should handle consecutively small add and borrows correctly (25x iterations before overflow)", async function () {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      try {
        await poolWethDai
          .connect(lp1)
          .addLiquidity(lp1.address, addAmount, timestamp + 1000000000, 0);
        numAdds += 1;
      } catch (error) {
        await expect(
          poolWethDai
    ...
    for (const loanId of loanIds) {
      await poolWethDai
        .connect(lp1)
        .claim(lp1.address, [loanId], false, timestamp + 9999999);
    }
    lpInfo = await poolWethDai.getLpInfo(lp1.address);
    await poolWethDai
      .connect(lp1)
