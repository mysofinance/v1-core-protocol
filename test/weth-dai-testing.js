...
describe("WETH-DAI Pool Testing", function () {
  ...
  it("Should not allow new LPs to claim on unentitled previous loans", async function () {
    ...
    await expect(
      poolWethDai.connect(lp4).claim(lp4.address, [1], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(poolWethDai, "UnentitledFromLoanIdx");
    await expect(
      poolWethDai.connect(lp4).claim(lp4.address, [2], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(poolWethDai, "UnentitledFromLoanIdx");
    await expect(
      poolWethDai.connect(lp4).claim(lp4.address, [3], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(poolWethDai, "UnentitledFromLoanIdx");
    await expect(
  ...
  it("(2/2) Should handle consecutively small add and borrows correctly (25x iterations before overflow)", async function () {
    ...
    await poolWethDai
      .connect(lp1)
      .removeLiquidity(lp1.address, lpInfo.sharesOverTime[lpInfo.sharesOverTime.length - 1]);
  });
});
