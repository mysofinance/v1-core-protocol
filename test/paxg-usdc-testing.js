...
describe("PAXG-USDC Pool Testing", function () {
  ...
  beforeEach(async () => {
    [deployer, lp1, lp2, lp3, lp4, lp5, borrower, pledger, ...addrs] =
    ...
    await PAXG.connect(supplyController).increaseSupply(
      "800000000000000000000000000"
    );
    await PAXG.connect(supplyController).transfer(
      borrower.address,
      "800000000000000000000000000"
    );
    await PAXG.connect(supplyController).increaseSupply(
    ...
    paxgPool = await PoolPaxg.deploy(
      _loanTenor,
      _maxLoanPerColl,
      _r1,
      _r2,
  ...
  it("Should fail on loan terms without LPs", async function () {
    await expect(paxgPool.loanTerms(ONE_PAXG)).to.be.revertedWithCustomError(
      paxgPool,
  ...
  it("Should not allow new LPs to claim on unentitled previous loans", async function () {
    ...
    await paxgPool
      .connect(lp1)
      .removeLiquidity(lp1.address, lp1InfoPre.sharesOverTime[0]);
    ...
    await expect(
      paxgPool
        .connect(lp1)
        .removeLiquidity(lp1.address, lp1InfoPre.sharesOverTime[0])
    ).to.be.revertedWithCustomError(paxgPool, "InvalidRemove");
    ...
    await expect(
      paxgPool.connect(lp4).claim(lp4.address, [1], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(paxgPool, "UnentitledFromLoanIdx");
    await expect(
      paxgPool.connect(lp4).claim(lp4.address, [2], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(paxgPool, "UnentitledFromLoanIdx");
    await expect(
      paxgPool.connect(lp4).claim(lp4.address, [3], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(paxgPool, "UnentitledFromLoanIdx");
    await expect(
      paxgPool
        .connect(lp4)
  ...
  it("Should be possible to borrow when there's sufficient liquidity, and allow new LPs to add liquidity to make borrowing possible again", async function () {
    ...
    for (let i = 0; i < 100; i++) {
      try {
        ...
        ).to.be.revertedWithCustomError(paxgPool, "LoanTooSmall");
        tooSmallLoans = true;
        break;
      }
    }
    ...
    await paxgPool
      .connect(borrower)
      .borrow(borrower.address, ONE_PAXG, 0, MONE, timestamp + 1000000000, 0);
  });
  ...
  it("Should handle aggregate claims correctly (1/2)", async function () {
    ...
    await expect(
      paxgPool
        .connect(lp1)
        .claimFromAggregated(lp1.address, [0, 99], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(paxgPool, "UnentitledFromLoanIdx");
    ...
    await expect(
      paxgPool
        .connect(lp2)
        .claimFromAggregated(lp2.address, [1, 99], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(paxgPool, "InvalidSubAggregation");
    await paxgPool
      .connect(lp2)
  ...
  it("Should allow removing liquidity", async function () {
    ...
    await expect(
      paxgPool
        .connect(lp1)
        .claimFromAggregated(lp1.address, [0, 199], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(paxgPool, "InvalidSubAggregation");
    await expect(
      paxgPool
        .connect(lp2)
    ...
    await paxgPool
      .connect(lp1)
      .removeLiquidity(lp1.address, lp1NumShares.sharesOverTime[0]);
    await paxgPool
      .connect(lp2)
      .removeLiquidity(lp2.address, lp2NumShares.sharesOverTime[0]);
    await paxgPool
      .connect(lp3)
      .removeLiquidity(lp3.address, lp3NumShares.sharesOverTime[0]);
  ...
  it("Should transfer dust when there are no more active LPs but previous LP claims and reinvests", async function () {
    ...
    await paxgPool
      .connect(lp1)
      .removeLiquidity(lp1.address, lpInfo.sharesOverTime[0]);
    ...
    await paxgPool
      .connect(lp1)
      .claim(lp1.address, [1], true, timestamp + 9999999);
  ...
  it("Should allow borrowing on behalf", async function () {
    ...
    await paxgPool
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_USDC.mul(10000), timestamp + 60, 0);
    ...
    await expect(
      paxgPool.connect(pledger).repay(1, borrower.address, loanInfo.repayment)
    ).to.be.revertedWithCustomError(paxgPool, "UnapprovedSender");
  ...
  it("Should allow adding liquidity on behalf", async function () {
    ...
    await expect(lp1Info.sharesOverTime[0]).to.be.equal(
      ONE_USDC.mul(10000).mul(1000).div(minLiquidity)
    );
  ...
  it("Should allow removing liquidity on behalf", async function () {
    ...
    await expect(
      paxgPool.connect(lp2).removeLiquidity(lp1.address, lpShares)
    ).to.be.revertedWithCustomError(paxgPool, "UnapprovedSender");
    ...
    await expect(postBal.sub(preBal)).to.be.equal(
      ONE_USDC.mul(10000).sub(minLiquidity)
    );
  });
  ...
  it("Should allow repaying on behalf", async function () {
    ...
    await expect(
      paxgPool.connect(lp1).repay(1, borrower.address, loanInfo.repayment)
    ).to.be.revertedWithCustomError(paxgPool, "UnapprovedSender");

    // should still fail with wrong approval
    await paxgPool
      .connect(borrower)
      .setApprovals(lp1.address, parseInt("11110", 2));
    await expect(
      paxgPool.connect(lp1).repay(1, borrower.address, loanInfo.repayment)
    ).to.be.revertedWithCustomError(paxgPool, "UnapprovedSender");
    ...
    await paxgPool
      .connect(borrower)
      .setApprovals(lp1.address, parseInt("00001", 2));
  ...
  it("Should allow rolling over on behalf", async function () {
    ...
    await paxgPool
      .connect(borrower)
      .setApprovals(lp1.address, parseInt("11101", 2));
    await expect(
      paxgPool
        .connect(lp1)
        .rollOver(1, 0, MAX_UINT128, timestamp + 1000000000, 0)
    ...
    await paxgPool
      .connect(borrower)
      .setApprovals(lp1.address, parseInt("00010", 2));
    preBalColl = await PAXG.balanceOf(lp1.address);
    preBalLoanCcy = await USDC.balanceOf(lp1.address);
    await paxgPool
      .connect(lp1)
      .rollOver(1, 0, MAX_UINT128, timestamp + 1000000000, rollOverCost);
  ...
  it("Should allow claiming on behalf", async function () {
    ...
    await paxgPool
      .connect(borrower)
      .repay(1, borrower.address, loanInfo1.repayment);
    ...
    await expect(
      paxgPool.connect(lp2).claim(lp1.address, [1], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(paxgPool, "UnapprovedSender");
    ...
    await expect(
      paxgPool.connect(lp1).setApprovals(lp2.address, parseInt("100001", 2))
    )
      .to.emit(paxgPool, "ApprovalUpdate")
      .withArgs(lp1.address, lp2.address, parseInt("00001", 2));

    // set correct approval
    await expect(
      paxgPool.connect(lp1).setApprovals(lp2.address, parseInt("10000", 2))
    )
      .to.emit(paxgPool, "ApprovalUpdate")
    ...
    expect(
      paxgPool.connect(lp1).setApprovals(lp2.address, parseInt("10000", 2))
    ).not.to.emit(paxgPool, "ApprovalUpdate");
    ...
    await paxgPool
      .connect(lp2)
      .claim(lp1.address, [1, 2], false, timestamp + 9999999);
    ...
    await expect(postBalLoanCcy.sub(preBalLoanCcy)).to.be.equal(
      loanInfo1.repayment
    );
  });
});
