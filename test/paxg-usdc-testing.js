...
describe('PAXG-USDC Pool Testing', function () {
  ...
  it('Should not allow new LPs to claim on unentitled previous loans', async function () {
    ...
    blocknum = await ethers.provider.getBlockNumber()
    timestamp = (await ethers.provider.getBlock(blocknum)).timestamp
    await ethers.provider.send('hardhat_setBalance', [borrower.address, '0x52B7D2DCC80CD2E4000000'])
    // borrow & repay
    loanTerms = await paxgPool.loanTerms(ONE_PAXG)
    minLoanLimit = loanTerms[0].mul(98).div(100)
    maxRepayLimit = loanTerms[1].mul(102).div(100)
    ...
    await expect(
      paxgPool.connect(lp1).removeLiquidity(lp1.address, lp1InfoPre.sharesOverTime[0])
    ...
    await expect(paxgPool.connect(lp4).claim(lp4.address, [1], false, timestamp + 9999999)).to.be.revertedWithCustomError(
      paxgPool,
      'UnentitledFromLoanIdx'
    )
    await expect(paxgPool.connect(lp4).claim(lp4.address, [2], false, timestamp + 9999999)).to.be.revertedWithCustomError(
      paxgPool,
      'UnentitledFromLoanIdx'
    )
    await expect(paxgPool.connect(lp4).claim(lp4.address, [3], false, timestamp + 9999999)).to.be.revertedWithCustomError(
      paxgPool,
      'UnentitledFromLoanIdx'
    )
    await expect(paxgPool.connect(lp4).claim(lp4.address, [1, 2], false, timestamp + 9999999)).to.be.revertedWithCustomError(
      paxgPool,
      'UnentitledFromLoanIdx'
    )
    await expect(paxgPool.connect(lp4).claim(lp4.address, [2, 3], false, timestamp + 9999999)).to.be.revertedWithCustomError(
      paxgPool,
      'UnentitledFromLoanIdx'
    )
    await expect(paxgPool.connect(lp4).claim(lp4.address, [1, 3], false, timestamp + 9999999)).to.be.revertedWithCustomError(
      paxgPool,
      'UnentitledFromLoanIdx'
    )
    await expect(
      paxgPool.connect(lp4).claim(lp4.address, [1, 2, 3], false, timestamp + 9999999)
  ...
  it("Should be possible to borrow when there's sufficient liquidity, and allow new LPs to add liquidity to make borrowing possible again", async function () {
    ...
    for (let i = 0; i < 100; i++) {
      try {
        ...
        await expect(
          paxgPool.connect(borrower).borrow(borrower.address, ONE_PAXG, 0, MONE, timestamp + 1000000000, 0)
  ...
  it('Should allow LPs to claim individually', async function () {
    ...
    // cannot claim twice
    await expect(
      paxgPool.connect(lp1).claim(lp1.address, loanIds, false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(paxgPool, 'UnentitledFromLoanIdx')
  ...
  it('Should handle aggregate claims correctly (1/2)', async function () {
    ...
    // cannot claim twice
    await expect(
      paxgPool.connect(lp1).claimFromAggregated(lp1.address, [0, 99], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(paxgPool, 'UnentitledFromLoanIdx')
    ...
    await expect(
      paxgPool.connect(lp2).claimFromAggregated(lp2.address, [1, 99], false, timestamp + 9999999)
    ...
    //cannot claim twice
    await expect(
      paxgPool.connect(lp2).claimFromAggregated(lp2.address, [0, 100], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(paxgPool, 'UnentitledFromLoanIdx')
  ...
  it('Should handle aggregate claims correctly (2/2)', async function () {
    ...
    await expect(
      paxgPool.connect(lp1).claimFromAggregated(lp1.address, [1, 3], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(paxgPool, 'InvalidSubAggregation')
    await paxgPool.connect(lp1).claim(lp1.address, [1, 2, 3], false, timestamp + 9999999)
    ...
    await expect(
      paxgPool.connect(lp3).claimFromAggregated(lp3.address, [1, 3], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(paxgPool, 'InvalidSubAggregation')
  ...
  it('Should allow removing liquidity', async function () {
    ...
    await expect(
      paxgPool.connect(lp1).claimFromAggregated(lp1.address, [0, 199], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(paxgPool, 'InvalidSubAggregation')
    await expect(
      paxgPool.connect(lp2).claimFromAggregated(lp2.address, [1, 99, 199], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(paxgPool, 'InvalidSubAggregation')
  ...
  it('Should allow borrowing on behalf', async function () {
    ...
    await expect(paxgPool.connect(pledger).repay(1, borrower.address, loanInfo.repayment)).to.be.revertedWithCustomError(
      paxgPool,
      'UnapprovedSender'
    )
  ...
  it('Should allow adding liquidity on behalf', async function () {
    ...
    await expect(
      paxgPool.connect(lp2).addLiquidity(lp1.address, ONE_USDC.mul(10000), timestamp + 60, 0)
  ...
  it('Should allow removing liquidity on behalf', async function () {
    ...
    await expect(paxgPool.connect(lp2).removeLiquidity(lp1.address, lpShares)).to.be.revertedWithCustomError(
      paxgPool,
      'UnapprovedSender'
    )
  ...
  it('Should allow repaying on behalf', async function () {
    ...
    await expect(paxgPool.connect(lp1).repay(1, borrower.address, loanInfo.repayment)).to.be.revertedWithCustomError(
      paxgPool,
      'UnapprovedSender'
    )
  ...
  it('Should allow rolling over on behalf', async function () {
    ...
    // check that lp is not entitled to repay
    await expect(paxgPool.connect(lp1).rollOver(1, 0, MAX_UINT128, timestamp + 1000000000, 0)).to.be.revertedWithCustomError(
      paxgPool,
      'UnapprovedSender'
    )
  ...
  it('Should allow claiming on behalf', async function () {
    ...
    await paxgPool
      .connect(borrower)
      .borrow(borrower.address, ONE_PAXG, loanTerms.loanAmount, loanTerms.repaymentAmount, timestamp + 60, 0)
    ...
    await expect(paxgPool.connect(lp2).claim(lp1.address, [1], false, timestamp + 9999999)).to.be.revertedWithCustomError(
      paxgPool,
      'UnapprovedSender'
    )
    ...
    await expect(paxgPool.connect(lp1).setApprovals(lp2.address, parseInt('100001', 2)))
      .to.emit(paxgPool, 'ApprovalUpdate')
      .withArgs(lp1.address, lp2.address, parseInt('00001', 2))

    // set correct approval
    await expect(paxgPool.connect(lp1).setApprovals(lp2.address, parseInt('10000', 2)))
      .to.emit(paxgPool, 'ApprovalUpdate')
