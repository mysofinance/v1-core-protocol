...
describe("PAXG-USDC Pool Testing", function () {
  ...
  it("Should not allow new LPs to claim on unentitled previous loans", async function () {
    ...
    await paxgPool
      .connect(borrower)
      .borrow(borrower.address, ONE_PAXG, minLoanLimit, maxRepayLimit, timestamp + 9999999, 0);
    ...
    await expect(
      paxgPool.connect(lp4).claim(lp4.address, [1, 2], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(paxgPool, "UnentitledFromLoanIdx");
    await expect(
      paxgPool.connect(lp4).claim(lp4.address, [2, 3], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(paxgPool, "UnentitledFromLoanIdx");
    await expect(
      paxgPool.connect(lp4).claim(lp4.address, [1, 3], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(paxgPool, "UnentitledFromLoanIdx");
    await expect(
  ...
  it("Should allow rolling over on behalf", async function () {
    ...
    // check that lp is not entitled to repay
    await expect(
      paxgPool.connect(lp1).rollOver(1, 0, MAX_UINT128, timestamp + 1000000000, 0),
    ).to.be.revertedWithCustomError(paxgPool, "UnapprovedSender");
  ...
  it("Should allow claiming on behalf", async function () {
    ...
    expect(paxgPool.connect(lp1).setApprovals(lp2.address, parseInt("10000", 2))).not.to.emit(
      paxgPool,
      "ApprovalUpdate",
    );
