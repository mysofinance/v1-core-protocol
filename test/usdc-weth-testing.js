...
describe("USDC-WETH Pool Testing", function () {
  ...
  beforeEach(async () => {
    ...
    poolUsdcWeth = await PoolUsdcWeth.deploy(
      _loanTenor,
      _maxLoanPerColl,
      _r1,
      _r2,
  ...
  it("Should fail on loan terms without LPs", async function () {
    await expect(poolUsdcWeth.loanTerms(ONE_ETH)).to.be.revertedWithCustomError(
      poolUsdcWeth,
  ...
  it("Should allow LPs to add liquidity", async function () {
    ...
    await poolUsdcWeth
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_ETH.mul(10), timestamp + 60, 0);
    await poolUsdcWeth
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_ETH.mul(100), timestamp + 60, 0);
    await poolUsdcWeth
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_ETH.mul(1000), timestamp + 60, 0);
    poolInfo = await poolUsdcWeth.getPoolInfo();
  ...
  it("Should allow borrowing with USDC", async function () {
    ...
    await poolUsdcWeth
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_ETH.mul(1000), timestamp + 60, 0);
    await poolUsdcWeth
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_ETH.mul(10000), timestamp + 60, 0);
    await poolUsdcWeth
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_ETH.mul(100000), timestamp + 60, 0);
  ...
  it("Should not allow new LPs to claim on unentitled previous loans", async function () {
    ...
    await poolUsdcWeth
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_ETH.mul(30000), timestamp + 60, 0);
    await poolUsdcWeth
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_ETH.mul(20000), timestamp + 60, 0);
    await poolUsdcWeth
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_ETH.mul(10000), timestamp + 60, 0);
    ...
    await expect(
      poolUsdcWeth
        .connect(lp1)
        .claim(lp1.address, [1, 2, 3], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolUsdcWeth, "UnentitledFromLoanIdx");
    ...
    await expect(
      poolUsdcWeth
        .connect(lp1)
        .removeLiquidity(lp1.address, lp1NumSharesPre.sharesOverTime[0])
    ).to.be.revertedWithCustomError(poolUsdcWeth, "InvalidRemove");
    ...
    await poolUsdcWeth
      .connect(lp4)
      .addLiquidity(lp4.address, ONE_ETH.mul(1000), timestamp + 60, 0);
    await expect(
      poolUsdcWeth
        .connect(lp4)
  ...
  it("Should be possible to borrow when there's sufficient liquidity, and allow new LPs to add liquidity to make borrowing possible again", async function () {
    ...
    await poolUsdcWeth
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_ETH.mul(2), timestamp + 60, 0);
    ...
    for (let i = 0; i < 100; i++) {
      try {
        ...
        ).to.be.revertedWithCustomError(poolUsdcWeth, "LoanTooSmall");
        tooSmallLoans = true;
        break;
      }
    }
    ...
    await poolUsdcWeth
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_ETH.mul(1000), timestamp + 60, 0);
  ...
  it("Should allow LPs to claim individually", async function () {
    ...
    await poolUsdcWeth
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_ETH.mul(100000), timestamp + 60, 0);
    await poolUsdcWeth
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_ETH.mul(100000), timestamp + 60, 0);
    await poolUsdcWeth
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_ETH.mul(100000), timestamp + 60, 0);
  ...
  it("Should handle aggregate claims correctly (1/2)", async function () {
    ...
    await poolUsdcWeth
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_ETH.mul(5000), timestamp + 60, 0);
    await poolUsdcWeth
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_ETH.mul(5000), timestamp + 60, 0);
    await poolUsdcWeth
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_ETH.mul(3000), timestamp + 60, 0);
    await poolUsdcWeth
      .connect(lp4)
      .addLiquidity(lp4.address, ONE_ETH.mul(2000), timestamp + 60, 0);
  ...
  it("Should handle aggregate claims correctly (2/2)", async function () {
    ...
    await poolUsdcWeth
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_ETH.mul(3000), timestamp + 60, 0);
    await poolUsdcWeth
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_ETH.mul(2000), timestamp + 60, 0);
    ...
    ).to.be.revertedWithCustomError(poolUsdcWeth, "InvalidSubAggregation");
    await poolUsdcWeth
      .connect(lp1)
      .claim(lp1.address, [1, 2, 3], false, timestamp + 9999999);
