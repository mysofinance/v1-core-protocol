...
describe("USDC-WETH Pool Testing", function () {
  ...
  beforeEach(async () => {
    ...
    await ethers.provider.send("hardhat_setBalance", [lp1.address, "0x204FCE5E3E25026110000000"]);
    ...
    await ethers.provider.send("hardhat_setBalance", [lp2.address, "0x204FCE5E3E25026110000000"]);
    ...
    await ethers.provider.send("hardhat_setBalance", [lp3.address, "0x204FCE5E3E25026110000000"]);
    ...
    await ethers.provider.send("hardhat_setBalance", [lp4.address, "0x204FCE5E3E25026110000000"]);
    ...
    await ethers.provider.send("hardhat_setBalance", [lp5.address, "0x204FCE5E3E25026110000000"]);
  ...
  it("Should allow borrowing with USDC", async function () {
    ...
    await poolUsdcWeth
      .connect(borrower)
      .borrow(borrower.address, ONE_USDC.mul(1000), minLoanLimit, maxRepayLimit, timestamp + 60, 0);
  });
  ...
  it("Should not allow new LPs to claim on unentitled previous loans", async function () {
    ...
    await poolUsdcWeth
      .connect(borrower)
      .borrow(borrower.address, ONE_USDC.mul(1500), minLoanLimit, maxRepayLimit, timestamp + 9999999, 0);
    ...
    // cannot claim twice
    await expect(
      poolUsdcWeth.connect(lp1).claim(lp1.address, [1, 2, 3], false, timestamp + 9999999),
    ...
    // cannot remove twice
    await expect(
      poolUsdcWeth.connect(lp1).removeLiquidity(lp1.address, lp1NumSharesPre.sharesOverTime[0]),
    ...
    await expect(
      poolUsdcWeth.connect(lp4).claim(lp4.address, [1], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(poolUsdcWeth, "UnentitledFromLoanIdx");
    await expect(
      poolUsdcWeth.connect(lp4).claim(lp4.address, [2], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(poolUsdcWeth, "UnentitledFromLoanIdx");
    await expect(
      poolUsdcWeth.connect(lp4).claim(lp4.address, [3], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(poolUsdcWeth, "UnentitledFromLoanIdx");
    await expect(
      poolUsdcWeth.connect(lp4).claim(lp4.address, [1, 2], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(poolUsdcWeth, "UnentitledFromLoanIdx");
    await expect(
      poolUsdcWeth.connect(lp4).claim(lp4.address, [2, 3], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(poolUsdcWeth, "UnentitledFromLoanIdx");
    await expect(
      poolUsdcWeth.connect(lp4).claim(lp4.address, [1, 3], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(poolUsdcWeth, "UnentitledFromLoanIdx");
    await expect(
      poolUsdcWeth.connect(lp4).claim(lp4.address, [1, 2, 3], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(poolUsdcWeth, "UnentitledFromLoanIdx");
  });
  ...
  it("Should be possible to borrow when there's sufficient liquidity, and allow new LPs to add liquidity to make borrowing possible again", async function () {
    ...
    for (let i = 0; i < 100; i++) {
      try {
        ...
        await poolUsdcWeth
          .connect(borrower)
          .borrow(borrower.address, ONE_USDC.mul(1000), 0, MAX_UINT128, timestamp + 1000000000, 0);
        numBorrows += 1;
  ...
  it("Should allow LPs to claim individually", async function () {
    ...
    // cannot claim twice
    await expect(
      poolUsdcWeth.connect(lp1).claim(lp1.address, loanIds, false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(poolUsdcWeth, "UnentitledFromLoanIdx");
  ...
  it("Should handle aggregate claims correctly (1/2)", async function () {
    ...
    for (let i = 0; i < 99; i++) {
      ...
      await poolUsdcWeth
        .connect(borrower)
        .borrow(borrower.address, pledgeAmount, 0, MAX_UINT128, timestamp + 1000000000, 0);
      // actual repayment
    ...
    // cannot claim twice
    await expect(
      poolUsdcWeth.connect(lp1).claimFromAggregated(lp1.address, [1, 100], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(poolUsdcWeth, "UnentitledFromLoanIdx");
    ...
    // cannot claim twice
    await expect(
      poolUsdcWeth.connect(lp2).claimFromAggregated(lp2.address, [0, 100], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(poolUsdcWeth, "UnentitledFromLoanIdx");
  ...
  it("Should handle aggregate claims correctly (2/2)", async function () {
    ...
    await expect(
      poolUsdcWeth.connect(lp1).claimFromAggregated(lp1.address, [1, 3], false, timestamp + 9999999),
    ...
    await expect(
      poolUsdcWeth.connect(lp2).claimFromAggregated(lp2.address, [1, 3], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(poolUsdcWeth, "InvalidSubAggregation");
    ...
    await expect(
      poolUsdcWeth.connect(lp3).claimFromAggregated(lp3.address, [1, 3], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(poolUsdcWeth, "InvalidSubAggregation");
