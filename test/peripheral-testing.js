...
describe("Peripheral testing", function () {
  ...
  beforeEach(async () => {
    ...
    peripheral = await Peripheral.deploy(
  ...
  it("Should revert when trying to borrow and repay within same block", async function () {
    ...
    await poolWethUsdc
      .connect(lp)
      .addLiquidity(lp.address, ONE_USDC.mul(100000), timestamp + 60, 0);
  ...
  it("Should revert when trying to atomically add liquidity and borrow", async function () {
    ...
    AddLiquidityAndBorrow = await ethers.getContractFactory(
    ...
    await poolWethUsdc
      .connect(borrower)
      .addLiquidity(borrower.address, addAmount, timestamp + 60, 0);
    pledgeAmount = ONE_ETH;
    ...
    await poolWethUsdc
      .connect(borrower)
      .removeLiquidity(borrower.address, lpInfo.sharesOverTime[0]);
