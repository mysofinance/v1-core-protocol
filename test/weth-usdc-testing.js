...
describe("WETH-USDC Pool Testing", function () {
  ...
  it("Should allow borrowing with ETH", async function () {
    ...
    await poolWethUsdc
      .connect(borrower)
      .borrow(borrower.address, ONE_ETH, minLoanLimit, maxRepayLimit, timestamp + 60, 0);
  });
  ...
  it("Should be possible to borrow when there's sufficient liquidity, and allow new LPs to add liquidity to make borrowing possible again", async function () {
    ...
    for (let i = 0; i < 100; i++) {
      try {
        ...
        await poolWethUsdc
          .connect(borrower)
          .borrow(borrower.address, ONE_ETH, 0, MAX_UINT128, timestamp + 1000000000, 0);
        numBorrows += 1;
  ...
  it("Should allow rolling over loan", async function () {
    ...
    await poolWethUsdc
      .connect(borrower)
      .rollOver(1, 0, MONE, timestamp + 1000000000, loanInfo.repayment.sub(loanTerms[0]));
  ...
  it("Should track loan index and share changes over time", async function () {
    ...
    await expect(
      poolWethUsdc.connect(lp1).claim(lp1.address, [], false, timestamp + 9999999),
    ).to.be.revertedWithCustomError(poolWethUsdc, "NothingToClaim");
    ...
    await poolWethUsdc
      .connect(lp1)
      .claim(lp1.address, [300, 310, 330, 340, currLoanIdx - 1], false, timestamp + 9999999);
  ...
  it("Should allow or revert approvals correctly", async function () {
    ...
    await expect(poolWethUsdc.connect(lp5).overrideSharePointer(1)).to.be.revertedWithCustomError(
      poolWethUsdc,
      "MustBeLp",
    );
