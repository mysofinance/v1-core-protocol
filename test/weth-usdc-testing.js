...
describe("WETH-USDC Pool Testing", function () {
  ...
  beforeEach(async () => {
    ...
    await WETH.connect(borrower).deposit({
      value: balance.sub(ONE_ETH.mul(10)),
    });
  ...
  it("Should fail on loan terms without LPs", async function () {
    await expect(poolWethUsdc.loanTerms(ONE_ETH)).to.be.revertedWithCustomError(
      poolWethUsdc,
  ...
  it("Should allow LPs to add liquidity", async function () {
    ...
    await poolWethUsdc
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_USDC.mul(1111), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_USDC.mul(10111), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_USDC.mul(130111), timestamp + 60, 0);
    poolInfo = await poolWethUsdc.getPoolInfo();
  ...
  it("Should allow borrowing with ETH", async function () {
    ...
    await poolWethUsdc
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_USDC.mul(1000), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_USDC.mul(10000), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_USDC.mul(100000), timestamp + 60, 0);
  ...
  it("Should not allow new LPs to claim on unentitled previous loans", async function () {
    ...
    await poolWethUsdc
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_USDC.mul(30000), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_USDC.mul(20000), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_USDC.mul(10000), timestamp + 60, 0);
    ...
    await poolWethUsdc
      .connect(borrower)
      .repay(2, borrower.address, loanTerms.repaymentAmount);
    ...
    await poolWethUsdc
      .connect(lp1)
      .claim(lp1.address, [1, 2, 3], false, timestamp + 9999999);
    // cannot claim twice
    await expect(
      poolWethUsdc
        .connect(lp1)
    ...
    await poolWethUsdc
      .connect(lp1)
      .removeLiquidity(lp1.address, lp1NumSharesPre.sharesOverTime[0]);
    ...
    await poolWethUsdc
      .connect(lp4)
      .addLiquidity(lp4.address, ONE_USDC.mul(1000), timestamp + 60, 0);
    await expect(
      poolWethUsdc
        .connect(lp4)
        .claim(lp4.address, [1], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolWethUsdc, "UnentitledFromLoanIdx");
    await expect(
      poolWethUsdc
        .connect(lp4)
        .claim(lp4.address, [2], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolWethUsdc, "UnentitledFromLoanIdx");
    await expect(
      poolWethUsdc
        .connect(lp4)
        .claim(lp4.address, [3], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolWethUsdc, "UnentitledFromLoanIdx");
    await expect(
      poolWethUsdc
        .connect(lp4)
        .claim(lp4.address, [1, 2], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolWethUsdc, "UnentitledFromLoanIdx");
    await expect(
      poolWethUsdc
        .connect(lp4)
        .claim(lp4.address, [2, 3], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolWethUsdc, "UnentitledFromLoanIdx");
    await expect(
      poolWethUsdc
        .connect(lp4)
        .claim(lp4.address, [1, 3], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolWethUsdc, "UnentitledFromLoanIdx");
    await expect(
      poolWethUsdc
        .connect(lp4)
        .claim(lp4.address, [1, 2, 3], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolWethUsdc, "UnentitledFromLoanIdx");
  });
  ...
  it("Should be possible to borrow when there's sufficient liquidity, and allow new LPs to add liquidity to make borrowing possible again", async function () {
    ...
    await poolWethUsdc
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_USDC.mul(1000), timestamp + 60, 0);
  ...
  it("Should allow LPs to claim individually", async function () {
    ...
    await poolWethUsdc
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_USDC.mul(100000), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_USDC.mul(100000), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_USDC.mul(100000), timestamp + 60, 0);
    ...
    for (let i = 0; i < 100; i++) {
      ...
      await poolWethUsdc
        .connect(borrower)
        .repay(i + 1, borrower.address, loanTerms.repaymentAmount);
    }
    loanIds = Array.from(Array(100), (_, index) => index + 1);

    await poolWethUsdc
      .connect(lp1)
      .claim(lp1.address, loanIds, false, timestamp + 9999999);
    // cannot claim twice
    await expect(
      poolWethUsdc
        .connect(lp1)
    ...
    await poolWethUsdc
      .connect(lp2)
      .claim(lp2.address, loanIds, false, timestamp + 9999999);
    await poolWethUsdc
      .connect(lp3)
      .claim(lp3.address, loanIds, false, timestamp + 9999999);
  });
  ...
  it("Should handle aggregate claims correctly (1/2)", async function () {
    ...
    await poolWethUsdc
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_USDC.mul(500000), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_USDC.mul(500000), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_USDC.mul(300000), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp4)
      .addLiquidity(lp4.address, ONE_USDC.mul(200000), timestamp + 60, 0);
    ...
    for (let i = 0; i < 99; i++) {
      ...
      // interest costs
      totalInterestCosts = totalInterestCosts.add(
        loanTerms[1].sub(loanTerms[0])
      );
      // repay
      await poolWethUsdc
        .connect(borrower)
        .repay(i + 1, borrower.address, loanInfo.repayment);
    }
    ...
    await expect(
      poolWethUsdc
        .connect(lp1)
        .claimFromAggregated(lp1.address, [1, 100], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolWethUsdc, "UnentitledFromLoanIdx");
    ...
    await poolWethUsdc
      .connect(lp2)
      .claimFromAggregated(lp2.address, [0, 100], false, timestamp + 9999999);
    ...
    await poolWethUsdc
      .connect(lp3)
      .claimFromAggregated(lp3.address, [0, 100], false, timestamp + 9999999);
    ...
    await poolWethUsdc
      .connect(lp4)
      .claimFromAggregated(lp4.address, [0, 100], false, timestamp + 9999999);
  ...
  it("Should handle aggregate claims correctly (2/2)", async function () {
    ...
    await poolWethUsdc
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_USDC.mul(300000), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_USDC.mul(200000), timestamp + 60, 0);
    ...
    await poolWethUsdc
      .connect(borrower)
      .repay(1, borrower.address, loanInfo.repayment);
    ...
    await poolWethUsdc
      .connect(borrower)
      .repay(2, borrower.address, loanInfo.repayment);
    ...
    await expect(
      poolWethUsdc
        .connect(lp1)
        .claimFromAggregated(lp1.address, [1, 3], false, timestamp + 9999999)
    ...
    await expect(
      poolWethUsdc
        .connect(lp2)
        .claimFromAggregated(lp2.address, [1, 3], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolWethUsdc, "InvalidSubAggregation");
    await poolWethUsdc
      .connect(lp2)
      .claim(lp2.address, [1, 2, 3], false, timestamp + 9999999);
    ...
    await expect(
      poolWethUsdc
        .connect(lp3)
        .claimFromAggregated(lp3.address, [1, 3], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolWethUsdc, "InvalidSubAggregation");
    await poolWethUsdc
      .connect(lp3)
      .claim(lp3.address, [1, 2, 3], false, timestamp + 9999999);
  ...
  it("Should allow removing liquidity", async function () {
    ...
    await poolWethUsdc
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_USDC.mul(10000000), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_USDC.mul(6000000), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_USDC.mul(4000000), timestamp + 60, 0);
    ...
    await poolWethUsdc
      .connect(lp1)
      .removeLiquidity(lp1.address, lp1Info.sharesOverTime[1]); // lp1 reinvested, so need to use newly pushed latest number of shares
    await poolWethUsdc
      .connect(lp2)
      .removeLiquidity(lp2.address, lp2Info.sharesOverTime[0]);
    await poolWethUsdc
      .connect(lp3)
      .removeLiquidity(lp3.address, lp3Info.sharesOverTime[0]);
  ...
  it("Should allow adding liquidity again after removing and claiming", async function () {
    ...
    await poolWethUsdc
      .connect(lp2)
      .claim(lp2.address, [1, 2, 3], true, timestamp + 9999999);
    await poolWethUsdc
      .connect(lp3)
      .claim(lp3.address, [1, 2, 3], false, timestamp + 9999999);
    ...
    await poolWethUsdc
      .connect(lp1)
      .removeLiquidity(lp1.address, lp1Info.sharesOverTime[0]);
    await poolWethUsdc
      .connect(lp2)
      .removeLiquidity(lp2.address, lp2Info.sharesOverTime[0]);
    await poolWethUsdc
    ...
    await poolWethUsdc
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_USDC.mul(500000), timestamp + 1000, 0);
    ...
    // check lp shares
    poolInfo = await poolWethUsdc.getPoolInfo();
    await expect(poolInfo._totalLpShares).to.be.equal(
      ONE_USDC.mul(500000).mul(1000).div(minLiquidity)
    );
  });
  ...
  it("Should never fall below minLiquidity", async function () {
    ...
    await poolWethUsdc
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_USDC.mul(1001), timestamp + 60, 0);
  ...
  it("Should allow rolling over loan", async function () {
    ...
    await expect(
      poolWethUsdc
        .connect(borrower)
        .rollOver(1, 0, MONE, timestamp + 1000000000, 0)
    ...
    await poolWethUsdc
      ...
      .rollOver(
        ...
        MONE,
        timestamp + 1000000000,
        loanInfo.repayment.sub(loanTerms[0])
      );
  ...
  it("Should track loan index and share changes over time", async function () {
    ...
    await poolWethUsdc
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_USDC.mul(100000000), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_USDC.mul(60000000), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_USDC.mul(40000000), timestamp + 60, 0);
    await poolWethUsdc
      .connect(lp4)
      .addLiquidity(lp4.address, ONE_USDC.mul(100000000), timestamp + 60, 0);
    ...
    await expect(initialLp1NumShares.sharesOverTime[0]).to.be.equal(
      poolInfo._totalLpShares.div(3)
    );

    await poolWethUsdc
      .connect(lp1)
      .removeLiquidity(lp1.address, initialLp1NumShares.sharesOverTime[0] / 2);
    await poolWethUsdc
      .connect(lp2)
      .removeLiquidity(lp2.address, initialLp2NumShares.sharesOverTime[0]);
    await poolWethUsdc
      .connect(lp3)
      .addLiquidity(lp3.address, ONE_USDC.mul(40000000), timestamp + 6000, 0);
    await poolWethUsdc
      .connect(lp4)
      .removeLiquidity(lp4.address, initialLp4NumShares.sharesOverTime[0]);
    ...
    await expect(secondLp1NumShares.sharesOverTime[1]).to.be.equal(
      secondLp1NumShares.sharesOverTime[0].div(2)
    );
    await expect(secondLp1NumShares.loanIdxsWhereSharesChanged[0]).to.be.equal(
      131
    );
    ...
    for (let i = 0; i < 150; i++) {
      try {
        ...
        await poolWethUsdc
          .connect(borrower)
          .repay(i + 131, borrower.address, loanInfo.repayment);
      } catch (error) {
        console.log(i, error);
      }
    }
    ...
    await poolWethUsdc
      .connect(lp2)
      .addLiquidity(lp2.address, ONE_USDC.mul(50000000), timestamp + 6000, 0);
    await poolWethUsdc
      .connect(lp4)
      .addLiquidity(lp4.address, ONE_USDC.mul(100000000), timestamp + 6000, 0);
    ...
    await expect(
      thirdLp2NumShares.loanIdxsWhereSharesChanged.length
    ).to.be.equal(2);
    await expect(thirdLp2NumShares.sharesOverTime[1]).to.be.equal(0);
    await expect(thirdLp2NumShares.loanIdxsWhereSharesChanged[0]).to.be.equal(
      131
    );
    await expect(thirdLp2NumShares.loanIdxsWhereSharesChanged[1]).to.be.equal(
      281
    );
    ...
    await expect(
      poolWethUsdc
        .connect(lp1)
        .claimFromAggregated(lp1.address, [0], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolWethUsdc, "NothingToClaim");
    await poolWethUsdc
      .connect(lp1)
      .claimFromAggregated(lp1.address, [0, 100], false, timestamp + 9999999);
    ...
    await expect(
      poolWethUsdc
        .connect(lp1)
        .claim(lp1.address, [98, 101, 102], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolWethUsdc, "UnentitledFromLoanIdx");
    await expect(
      poolWethUsdc
        .connect(lp1)
        .claim(lp1.address, [100, 103, 102], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolWethUsdc, "NonAscendingLoanIdxs");

    await expect(
      poolWethUsdc
        .connect(lp1)
        .claim(lp1.address, [], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolWethUsdc, "NothingToClaim");
    await poolWethUsdc
      .connect(lp1)
      .claim(
        lp1.address,
        [100, 103, 104, 105, 108, 112, 120],
        false,
        timestamp + 9999999
      );
    ...
    await poolWethUsdc
      .connect(lp1)
      .claimFromAggregated(lp1.address, [200, 300], false, timestamp + 9999999);
    ...
    await expect(
      poolWethUsdc
        .connect(lp2)
        .claim(lp1.address, [300, 310], false, timestamp + 9999999)
    ...
    await poolWethUsdc
      ...
      .addLiquidity(
        lp1.address,
        ONE_USDC.mul(40000000),
        timestamp + 60 * 60 * 24 * 365 + 900,
        0
      );
    ...
    await expect(thirdLp1NumShares.loanIdxsWhereSharesChanged[1]).to.be.equal(
      currLoanIdx
    );
    ...
    console.log(
      `lp2 ${currLp2NumShares.loanIdxsWhereSharesChanged[0].toString()}`
    );
    console.log(
      `lp2 ${currLp2NumShares.loanIdxsWhereSharesChanged[1].toString()}`
    );
    ...
    await expect(
      poolWethUsdc
        .connect(lp2)
        .claim(lp2.address, [131, 140, 250], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolWethUsdc, "UnentitledFromLoanIdx");
    ...
    await poolWethUsdc
      .connect(lp4)
      .removeLiquidity(lp4.address, thirdLp4NumShares.sharesOverTime[2]);
    ...
    await expect(
      poolWethUsdc
        .connect(lp4)
        .claim(lp4.address, [0, 130], false, timestamp + 9999999)
    ).to.be.revertedWithCustomError(poolWethUsdc, "InvalidLoanIdx");

    await poolWethUsdc
      .connect(lp4)
      .claim(lp4.address, [1, 130], false, timestamp + 9999999);
    ...
    for (let i = 0; i < 10; i++) {
      try {
        ...
        await poolWethUsdc
          .connect(borrower)
          .repay(currLoanIdx.add(i), borrower.address, loanInfo.repayment);
      } catch (error) {
        console.log(i, error);
      }
    }
  ...
  it("Should allow or revert approvals correctly", async function () {
    ...
    await expect(
      poolWethUsdc.connect(lp2).setApprovals(lp2.address, parseInt("10011", 2))
    ...
    await poolWethUsdc
      .connect(lp2)
      .setApprovals(lp1.address, parseInt("11100", 2));
    ...
    await expect(
      poolWethUsdc.connect(lp5).overrideSharePointer(1)
    ).to.be.revertedWithCustomError(poolWethUsdc, "MustBeLp");
    // from curr share pointer greater than or equal to 1
    await expect(
      poolWethUsdc.connect(lp1).overrideSharePointer(1)
    ).to.be.revertedWithCustomError(poolWethUsdc, "InvalidNewSharePointer");
    // new pointer not less than length of shares over time array
    await expect(
      poolWethUsdc.connect(lp3).overrideSharePointer(4)
    ).to.be.revertedWithCustomError(poolWethUsdc, "InvalidNewSharePointer");

    await poolWethUsdc
      .connect(lp3)
      .claim(lp3.address, [1, 5, 9, 10], false, timestamp + 9999999);
    ...
    await expect(lp3Info.currSharePtr).to.be.equal(1);
    expect(
      thirdlp3NumShares.sharesOverTime[1].gt(
        secondlp3NumShares.sharesOverTime[1]
      )
    );
    ...
    for (let i = 0; i < 10; i++) {
      try {
        ...
        await poolWethUsdc
          .connect(borrower)
          .repay(i + 11, borrower.address, loanInfo.repayment);
      } catch (error) {
        console.log(i, error);
      }
    }
    ...
    await expect(lp3Info.currSharePtr).to.be.equal(1);
    expect(
      fourthlp3NumShares.sharesOverTime[2].gt(
        thirdlp3NumShares.sharesOverTime[1]
      )
    );
    ...
    await expect(
      poolWethUsdc
        ...
        .removeLiquidity(
          lp3.address,
          fourthlp3NumShares.sharesOverTime[2].sub(
            fourthlp3NumShares.sharesOverTime[1]
          )
        )
    ).to.be.revertedWithCustomError(poolWethUsdc, "BeforeEarliestRemove");
    // go ahead and revert non lp case
    await expect(
      poolWethUsdc.connect(lp5).removeLiquidity(lp5.address, 100)
    ).to.be.revertedWithCustomError(poolWethUsdc, "InvalidRemove");
    ...
    await expect(
      poolWethUsdc
        .connect(lp3)
        .removeLiquidity(lp3.address, fourthlp3NumShares.sharesOverTime[2])
    ).to.be.revertedWithCustomError(poolWethUsdc, "InvalidRemove");
  ...
  it("Should revert on repay or rollover invalid transactions", async function () {
    ...
    await expect(
      poolWethUsdc
        .connect(borrower)
        .repay(0, borrower.address, loanInfo.repayment)
    ).to.be.revertedWithCustomError(poolWethUsdc, "InvalidLoanIdx");
    await expect(
      poolWethUsdc.connect(lp1).repay(1, lp2.address, loanInfo.repayment)
    ).to.be.revertedWithCustomError(poolWethUsdc, "InvalidRecipient");
    await expect(
      poolWethUsdc
        .connect(borrower)
        .repay(1, borrower.address, loanInfo.repayment.div(2))
    ).to.be.revertedWithCustomError(poolWethUsdc, "InvalidSendAmount");
    ...
    await expect(
      poolWethUsdc
        .connect(borrower)
        .rollOver(0, 0, MAX_UINT128, timestamp + 1000000000, 0)
    ).to.be.revertedWithCustomError(poolWethUsdc, "InvalidLoanIdx");
    await expect(
      poolWethUsdc
        .connect(borrower)
        .rollOver(1, 0, MAX_UINT128, timestamp + 1000000000, 0)
    ).to.be.revertedWithCustomError(poolWethUsdc, "AlreadyRepaid");
    ...
    await expect(
      poolWethUsdc
        .connect(borrower)
        .rollOver(2, 0, MAX_UINT128, timestamp + 1000000000, 0)
    ).to.be.revertedWithCustomError(poolWethUsdc, "CannotRepayAfterExpiry");
  });
  ...
  it("should revert on expired last claim and claim not settled", async function () {
    ...
    await expect(
      poolWethUsdc
        .connect(lp1)
        .claim(lp1.address, [1], false, timestamp + 1000000000)
    ...
    await expect(
      poolWethUsdc.connect(lp1).getClaimsFromAggregated(0, 100, 100)
    ).to.be.revertedWithCustomError(poolWethUsdc, "InvalidSubAggregation");
  });
  ...
  it("should allow override share pointer and revert on small add", async function () {
    ...
    timestamp = (await ethers.provider.getBlock(blocknum)).timestamp;
    await expect(
      poolWethUsdc.connect(lp1).addLiquidity(lp1.address, 10, timestamp + 60, 0)
    ).to.be.revertedWithCustomError(poolWethUsdc, "InvalidAddAmount");
    await poolWethUsdc
      .connect(lp1)
      .addLiquidity(lp1.address, ONE_USDC.mul(100000000), timestamp + 60, 0);
  ...
  it("Should allow claiming on 10,000 loans", async function () {
    ...
    for (let i = 0; i < 5000; i++) {
      try {
        ...
        loanInfo = await poolWethUsdc.loanIdxToLoanInfo(i + 1);
        aggregateCollPerShare = aggregateCollPerShare.add(
          loanInfo[1].mul(MONE).div(poolInfo._totalLpShares)
        );
      } catch (error) {
        console.log(i, error);
      }
    ...
    for (let i = 0; i < 4999; i++) {
      try {
        ...
        aggregateRepaymentsPerShare = aggregateRepaymentsPerShare.add(
          loanInfo[0].mul(MONE).div(poolInfo._totalLpShares)
        );
        await poolWethUsdc
          .connect(borrower)
          .repay(i + 5001, borrower.address, loanInfo.repayment);
      } catch (error) {
        console.log(i, error);
      }
    }
    ...
    const firstLpShares = ONE_USDC.mul(1000000000000)
      .mul(1000)
      .div(minLiquidity);
    expect(postClaimBalCollCcy.sub(preClaimBalCollCcy)).to.be.equal(
      aggregateCollPerShare.mul(firstLpShares).div(MONE)
    );
    expect(postClaimBalLoanCcy.sub(preClaimBalLoanCcy)).to.be.equal(
      aggregateRepaymentsPerShare.mul(firstLpShares).div(MONE)
    );
  });
});
